<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Authentication Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used during authentication for various
    | messages that we need to display to the user. You are free to modify
    | these language lines according to your application's requirements.
    |
    */

    // 'failed'   => 'Ці облікові дані не збігаються з нашими записами.',
    'failed' => 'Невірний E-mail або пароль',
    'throttle' => 'Занадто багато спроб входу. Будь ласка, спробуйте ще раз, через :seconds секунд.',

    'login' => 'Вхід',
    'logout' => 'Вихід',
    'login-form-title' => 'Вхід в мій кабінет',

    'email-address' => 'E-Mail адреса',
    'password' => 'Пароль',
    'password-conf' => 'Підтвердження пароля',
    'reset-password' => 'Скинути пароль',
    'remember-me' => "Запам'ятай мене",
    'cancel' => 'Відміна',
    'reset' => 'Cкинути',
    'close' => 'Закрити',
    'rem-password' => 'Я згадав пароль',
    'register' => 'Зареєструватись',

    'username' => "Ваше ім'я",
    'company' => 'Компанія',
    'phone' => 'Телефон',

    'more-contacts' => 'Додаткові контактні дані',
    'my-office' => 'Мій кабінет',
    'not-auth' => 'Авторизуйтесь для здійснення даної дії.',

    // ну да! такое уже есть, но звучит по другому)
    'password-reset' => 'Скидання пароля',
    'password-click-here' => 'Натисніть тут, щоб скинути пароль',
];
